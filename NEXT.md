...
## #318 — rounding is done in decimal, not at a binary precision: `sp.N(x, digits)` misrounds the last digit — **package half done 6 Sep 2026, committed and pushed, unreleased (rides 0.5.31); app half open, awaiting Roberto's go**
...
**The app half, open.** `symbulator_ui.py`'s `_round_expr` (five call
sites), `_polar_format` and the `aa` mini-tool should call the
package's `round_sig` once 0.5.31 is pinned -- the server cannot
import it before the release. Expect the twelve answers above to move
in the last digit on every site, and `tools/*.expected.json` (18
books, generated by the app) to need regenerating for those twelve;
the book's printed answers, not the expected files, are the truth to
check against. The offline pair carries the same code and moves with
the same build. Nothing in the docs changes: the tutorial prints the
book's values, which are the correct ones.
