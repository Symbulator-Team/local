...
## #429 — claimed by the docs tree, 12 Sep 2026: **the Nilsson & Riedel sampler, made to read well** — a prose-only pass over `/9/nr12-sampler`: every `ask` checked against the book's own PDF text, every `shows` paragraph rewritten, seven rendering faults in the generated page fixed, and the forty-three-fold *the same answers the book prints* replaced by one claim in *How to read an entry*. **One app-side file moves with it:** `examples/Nilsson_Riedel.cir`, whose `note:` lines are the page's `ask` texts, regenerated by the same `gen.py`; no circuit, setting or answer changes, and all 43 pass `verify_lesson.py` through the real app. Built, not deployed; when it goes it is a cache bump and the offline pair plus a pull on both PythonAnywhere accounts, no `pip`. Write-up in `Documentation/NEXT_DOCS.md`
